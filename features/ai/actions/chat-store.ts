...
import { MessageRole, Prisma } from "@/prisma/generated/prisma/client";
...
export async function loadChatMessages(
  ...
  return rows.map((row) => ({
    id: row.id,
    role: row.role === MessageRole.ASSITANT ? "assistant" : "user",
...
export async function saveChatMessages(
  ...
  for (const message of messages) {
    ...
    const role = message.role === "assistant" ? "ASSITANT" : "USER";
